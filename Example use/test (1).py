...
def draw_bounding_box(img, class_id, confidence, x, y, x_plus_w, y_plus_h):
    ...
    LABLE.append(label)
    CONFIDENT.append(confidence)
    cv2.rectangle(img, (int(x),int(y)), (int(x_plus_w),int(y_plus_h)), color, 2)
    cv2.putText(img, label+":"+str(confidence), (int(x-10),int(y-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
...
############################## Check ears o the pig #####################################
classes = None
with open("pig.names", 'r') as f:
    classes = [line.strip() for line in f.readlines()]

# generate different colors for different classes 
COLORS = np.random.uniform(0, 255, size=(len(classes), 3))
# read pre-trained model and config file
net = cv2.dnn.readNet("pig_f_b_as.weights", "pig_f_b_as.cfg")
# create input blob 
...
classes = None
with open("Pig_su.names", 'r') as f:
    classes = [line.strip() for line in f.readlines()]

# generate different colors for different classes 
COLORS = np.random.uniform(0, 255, size=(len(classes), 3))
# read pre-trained model and config file
net1 = cv2.dnn.readNet("pig_side_up.weights", "pig_side_up.cfg")
# create input blob 
...
print(LABLE[CONFIDENT.index(max(CONFIDENT))])
